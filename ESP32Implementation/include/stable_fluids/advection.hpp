...
namespace stable_fluids{
    ...
    void advect(float (&d)[NX+2][NY+2], const float (&d0)[NX+2][NY+2],
                float (&u)[NX+2][NY+2], const float (&v)[NX+2][NY+2],
                int b, FluidParams<NX, NY>& params) {
                    ...
                    for (size_t i = 1; i <= NX; i++) {
                        for (size_t j = 1; j <= NY; j++) {
                            ...
                            // std::clamp necesita que los 3 argumentos sean del mismo tipo, por lo que hay que castear
